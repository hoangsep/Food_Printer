...
// Start modifying, mix mode for food printer
			...
			if (Extruder::mixMode) {
				...
				if (diff1 <= diff2) {
					...
					else { // diff3 < diff1 < diff2
// If there is no extruder 3 => no need to step
// Still need to increase the ext3_steps to avoid infinite loop
// That scenario should not happen (set the extruder ratio to non-zero while don't have extruder 3)
					...
					if (diff2 <= diff3) { // diff2 < diff1 and diff2 < diff3}
						WRITE(EXT1_STEP_PIN, HIGH);
						ext2_steps++;
					}
					else { // diff3 < diff2 < diff1
#if defined(EXT2_STEP_PIN) && NUM_EXTRUDER>2
						WRITE(EXT2_STEP_PIN, HIGH);
#endif
						ext3_steps++;
				...
				// reset the counters if 100 steps are reached
				current_total_steps++;
				if (current_total_steps == 100) {
					ext1_steps = 0;
					ext2_steps = 0;
					ext3_steps = 0;
